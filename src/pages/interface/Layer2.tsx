...
const Layer2: React.FC<Layer2Props> = ({buttonActiveRule, setSelectedRule, selectedRule}) => {
  return (
    ...
    <ul hidden={buttonActiveRule !== "1"}>
        ...
        <ButtonProposition questionPropostionDiv={false} color="" setListSelect={()=>{}} listSelect={[]} id={Number(content.id)} index={`${index}`} method={content.method} name={content.name} select={selectedRule} onClick={setSelectedRule} type='default' typeActive={false}  />
        ))}
    </ul>
